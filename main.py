...
def noise_free_learn(list_of_data,status):
    if list_of_data == None:
        ...
        for i in range(len(lis_of_data)):
            for j in range(lis_of_data[i]):
                ...
                canv.create_text(pointx,pointy,text=f"{status[i][j][0]}",fill = "blue",font = ("Helvetica", 20))
        ...
        buttonnext = Button(m,background='blue',text="Next",font=("Helvetica", 20),foreground='white',command=partial(noise_free_learn,lis_of_data))
...
def main():
    ...
    buttonnoisefree = Button(m,background='red',text="Noise Free",font=("Helvetica", 20),foreground='white',command=partial(noise_free_learn,None,None))
